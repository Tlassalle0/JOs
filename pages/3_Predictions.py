...
combin= swim_df.copy()
combin=combin['Distance','Stroke']
combin=combin.drop_duplicates()
...
with tab1:
    ...
    if swim_df.empty or "Stroke" not in swim_df.columns:
        ...
        with c2:
            ...
            stroke = st.selectbox(
                "Style", options=stroke_options,
                index=stroke_options.index("Freestyle") if "Freestyle" in stroke_options else 0,
            )
        with c3:
            st.markdown("<div style='height:2rem'></div>", unsafe_allow_html=True)
            predict_btn = st.button("Prédire le Top 10", key="top10_btn", use_container_width=True)
    ...
    if swim_df.empty or "Stroke" not in swim_df.columns or "Gender" not in swim_df.columns:
        ...
        with st.form("prediction_form"):
            ...
            with c1:
                ...
                stroke_opts = sorted(swim_df["Stroke"].dropna().unique().tolist())
                stroke = st.selectbox("Stroke", stroke_opts, key="m_stroke")
